...
func (g *GatherJob) GatherAndUpload(kubeConfig, protoKubeConfig *rest.Config) error { // nolint: funlen, gocyclo
	...
	// TODO: this whole functionality needs to be replaced with conditions - since we have removed the state ~ phase
	// dataGatherCR, err = status.UpdateDataGatherState(ctx, insightsv1alpha2client, dataGatherCR, insightsv1alpha1.Running)
	// if err != nil {
...
func gatherAndReporFunctions(ctx context.Context, gatherersToRun []gatherers.Interface, // nolint: gocritic
	...
	for _, gatherer := range gatherersToRun {
		// TODO: should be here more checks? at least if Gatherers is not nil?
		functionReports, err := gather.CollectAndRecordGatherer(ctx, gatherer, rec, dataGatherCR.Spec.Gatherers.Custom.Configs) // nolint: govet
		if err != nil {
			klog.Errorf("unable to process gatherer %v, error: %v", gatherer.GetName(), err)
		}
...
func updateDataGatherStatus(ctx context.Context, insightsClient insightsv1alpha2client.InsightsV1alpha2Interface,
	...
	dataGatherUpdated, err := status.UpdateDataGatherState(ctx, insightsClient, dataGatherCR, gatheringStatus)
	if err != nil {
