...
func GatherClusterOperators(i *Gatherer) func() ([]record.Record, []error) {
	return func() ([]record.Record, []error) {
		...
		for idx, co := range config.Items {
			...
			relRes := collectClusterOperatorResources(i.dynamicClient, co, resVer)
...
func collectClusterOperatorResources(dynamicClient dynamic.Interface, co configv1.ClusterOperator, resVer map[string][]string) []clusterOperatorResource {
	...
	for _, ro := range relObj {
		...
		for _, v := range versions {
			...
			clusterResource, err := dynamicClient.Resource(gvr).Get(ro.Name, metav1.GetOptions{})
			if err != nil {
