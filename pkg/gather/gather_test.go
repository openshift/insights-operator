...
func Test_CollectAndRecordGatherer(t *testing.T) {
	...
	anonymizer, err := anonymization.NewAnonymizer("", nil, nil, mockSecretConfigurator, nil)
	assert.NoError(t, err)
...
func Test_CollectAndRecordGatherer_Error(t *testing.T) {
	...
	anonymizer, err := anonymization.NewAnonymizer("", []string{}, nil, config.NewMockSecretConfigurator(nil), nil)
	assert.NoError(t, err)
	err = RecordArchiveMetadata(functionReports, mockRecorder, anonymizer)
	assert.NoError(t, err)
