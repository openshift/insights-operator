...
func NewNetworkAnonymizerFromConfig(
	...
	return NewNetworkAnonymizerFromConfigClient(ctx,
...
func NewNetworkAnonymizerFromConfigClient(
	...
	for value, placeholder := range sensitiveVals {
		networkAnonymizerBuilder.WithSensitiveValue(value, placeholder)
	}
	...
	networkAnonymizerBuilder.WithSensitiveValue(baseDomain, ClusterBaseDomainPlaceholder)

	return networkAnonymizerBuilder.Build()
}

func (na *NetworkAnonymizer) GetType() AnonymizerType {
	return NetworkAnonymizerType
}
...
func (na *NetworkAnonymizer) AnonymizeData(memoryRecord *record.MemoryRecord) (*record.MemoryRecord, error) {
	...
	once.Do(func() {
		err := na.readNetworkConfigs()
		if err != nil {
			klog.Errorf("failed to read network config: %v", err)
		}
	})

	for value, placeholder := range na.sensitiveValues {
	...
	memoryRecord.Data = na.ipNetworkRegex.ReplaceAllFunc(memoryRecord.Data, func(originalIPBytes []byte) []byte {
		return []byte(na.ObfuscateIP(string(originalIPBytes)))
	})
...
func (na *NetworkAnonymizer) ObfuscateIP(ipStr string) string {
	...
	if obfuscatedIP, exists := na.translationTable[ipStr]; exists {
		return obfuscatedIP
	}
	...
	for i := range na.networks {
		networkInfo := &na.networks[i]
		...
		if network.Contains(originalIP) {
			...
			na.translationTable[ipStr] = nextIP.String()
...
func (na *NetworkAnonymizer) StoreTranslationTable() *corev1.Secret {
	if len(na.translationTable) == 0 {
		return nil
	}
	defer na.ResetTranslationTable()
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	err := na.secretsClient.Delete(ctx, TranslationTableSecretName, metav1.DeleteOptions{})
	if err != nil {
	...
	secret := corev1.Secret{
		...
		StringData: na.translationTable,
	}
	...
	result, err := na.secretsClient.Create(ctx, &secret, createOptions)
	if err != nil {
...
func (na *NetworkAnonymizer) ResetTranslationTable() {
	na.translationTable = make(map[string]string)
}
...
func (na *NetworkAnonymizer) IsEnabled() bool {
	obfuscation := na.configurator.Config().DataReporting.Obfuscation
...
func (na *NetworkAnonymizer) readNetworkConfigs() error {
	...
	networksConfig, err := na.configClient.Networks().Get(context.Background(), "cluster", metav1.GetOptions{})
	if err != nil {
		return err
	}

	clusterConfigV1, err := na.gatherKubeClient.CoreV1().
	...
	hostSubnets, err := na.networkClient.HostSubnets().List(context.Background(), metav1.ListOptions{})
	if err != nil {
	...
	for _, network := range cidrs {
		lastIP := network.IP
		na.networks = append(na.networks, subnetInformation{
			network: *network,
			lastIP:  lastIP,
		})
	}
