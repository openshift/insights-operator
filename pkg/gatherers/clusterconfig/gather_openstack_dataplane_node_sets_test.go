...
func assertFieldValue(t *testing.T, data map[string]interface{}, expectedValue string, fields ...string) {
	...
	if !found {
		t.Fatal(fmt.Sprintf("Field '%s' was not found in the gathered object", fields))
	}
	if err != nil {
		t.Fatal(err)
	}
...
`},
			exp: []string{"customresources/dataplane.openstack.org/openstackdataplanenodesets/openstack/test-cr"},
		},
		{
...
`},
			exp: []string{
				"customresources/dataplane.openstack.org/openstackdataplanenodesets/openstack/test-cr-1",
				"customresources/dataplane.openstack.org/openstackdataplanenodesets/openstack/test-cr-2",
			},
		},
	}
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := range records {
				...
				if len(test.osdpnsYAML) > 0 {
					...
					assertFieldValue(t, gatheredItem.Object, "xxxxxxxxx", "spec", "customService", "ip_address")
