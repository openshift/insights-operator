...
// GatherActiveAlerts gathers active alerts from the Alertmanager API V2.
...
func gatherActiveAlerts(ctx context.Context, alertsClient rest.Interface) ([]record.Record, []error) {
	...
	if len(alerts) > alertLimit {
		...
		errs = append(errs, fmt.Errorf("alert limit %d was exceeded! There was %d alerts", alertLimit, originalCount))
	}
