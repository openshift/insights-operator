...
- under auths object in property console.redhat.com is property auth with Bearer token for console.redhat.com Authentication.
...
add the Bearer token, which is read from secret pull-secret, the section .auths.console.redhat.com.auth. The clientTransport is also setting Proxy, which
