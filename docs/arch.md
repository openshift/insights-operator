...
to `console.redhat.com` for analysis.
...
- under auths object in property `cloud.openshift.com` is property auth with Bearer token for `console.redhat.com` Authentication.
...
To configure authentication to `console.redhat.com` Insights Operator is reading preconfigured token from namespace
`openshift-config` and secret `pull-secret` (where are cluster-wide tokens stored). The token to `console.redhat.com` is stored in .dockerjsonconfig, inside auth section.
```shell script
...
# How is Uploader authenticating to console.redhat.com
...
add the Bearer token, which is read from secret `pull-secret`, the section .auths.cloud.openshift.com.auth. The clientTransport is also setting Proxy, which
